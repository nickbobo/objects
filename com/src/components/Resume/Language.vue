<template>
  <div class="language">
    <el-row class="title_def_style r-title">
      <el-col :span="23" class="user-headimg title_font_mix18">语言能力</el-col>
    </el-row>

    <span v-if="!isListNull && add_resume_id" class="add-line" @click="onAdd">
      <span v-if="resume_status">添加语言能力</span>
    </span>
    <div class="r-bd" v-if="isListNull">
      <div class="row edu-row">
        <div class="col-2-1" v-for="item in dataList" :key="item.Id">
          <div class="edu-item">
            <h3>
              {{ item.LanguageName | IsNull | beautySub(20) }}
              <!-- <label class="label">统招</label> -->
            </h3>
            <div class="info">
              <span class="major"> {{ item.ReadWriteAbility | IsNull }} </span>
            </div>
            ...
            <button v-if="resume_status" type="button" class="r-fun r-del" @click="$refs.isDelDialog.showIsDel(item)">
    ...
    <is-del-dialog ref="isDelDialog" :titles="'语言能力'" @callbackFun="onDel"></is-del-dialog>
    <el-dialog title="语言能力" :visible.sync="isShowdialog" class="edit_dialog education_experience" :modal-append-to-body="false" :close-on-click-modal="false" @close="$reset('fromData')">
      <el-form ref="fromData" label-position="top" :rules="ruleValidate" label-width="80px" :model="fromData" size="small">
        <el-row>
          <el-form-item label="语言种类" prop="LanguageName">
            <el-input type="text" maxlength="35" show-word-limit v-model="fromData.LanguageName" placeholder="请输入语言种类"></el-input>
          </el-form-item>
          <el-form-item label="熟练程度" prop="ReadWriteAbility">
            <el-select v-model="fromData.ReadWriteAbility" label="熟练程度" placeholder="熟练程度" prop="Gender">
              <el-option :label="item.name" :value="item.name" v-for="(item, index) in selectList" :key="item.name"></el-option>
            </el-select>
          </el-form-item>
          </el-col>
        </el-row>
...
<script>
  import { GetLanguage, DelLanguage, EditLanguage } from "@/api/myResume";
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        showData: {
          "ResumeId": "string",
          "LanguageName": "string",
          "ListenSpeakAbility": "string",
          "ReadWriteAbility": "string",
        ...
        ruleValidate: {
          ...
          Institutions: [{ required: true, message: "请输入语言种类", trigger: "change" },],
    ...
    methods: {
      ...
      getList() {
        GetLanguage({ ResumeId: this.add_resume_id }).then((res) => {
          if (res.data.Success) {
      ...
      editAction() {
        this.$refs["fromData"].validate((valid) => {
          if (valid) {
            ...
            let data = {
              language: this.fromData,
              type: this.saveStatus, //1 新增   2 修改
            };

            EditLanguage(data).then((res) => {
              if (res.data.Success) {
      ...
      onDel(item) {
        DelLanguage({ Id: item.Id }).then((res) => {
          if (res.data.Success) {
            this.$message({
              message: "删除成功",
              type: "success",
