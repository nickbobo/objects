<template>
  <div class="train">
    <el-row class="title_def_style r-title">
      <el-col :span="23" class="user-headimg title_font_mix18">培训经历</el-col>
    </el-row>

    <span v-if="!isListNull && add_resume_id" class="add-line" @click="onAdd">
      <span v-if="resume_status">添加培训经历</span>
    </span>
    <div class="r-bd" v-if="isListNull">
      <div class="row edu-row">
        <div class="col-2-1" v-for="item in dataList" :key="item.Id">
          <div class="edu-item">
            <h3>
              {{ item.Course | IsNull | beautySub(20) }}
    ...
    <is-del-dialog ref="isDelDialog" :titles="'培训经历'" @callbackFun="onDel"></is-del-dialog>
    <el-dialog title="培训经历" :visible.sync="isShowdialog" class="edit_dialog education_experience" :modal-append-to-body="false" :close-on-click-modal="false" @close="$reset('fromData')">
      <el-form ref="fromData" label-position="top" :rules="ruleValidate" label-width="80px" :model="fromData" size="small">
        <el-row>
          <el-form-item label="课程名称" prop="Course">
            <el-input type="text" maxlength="35" show-word-limit v-model="fromData.Course" placeholder="请输入课程名称"></el-input>
          </el-form-item>
          <el-form-item label="机构名称" prop="Institutions">
            <el-input type="text" maxlength="35" show-word-limit v-model="fromData.Institutions" placeholder="请输入机构名称"></el-input>
          </el-form-item>
          <el-form-item label="培训时间" class="start-end-time" prop="StartTime">
            <el-date-picker v-model="fromData.StartTime" type="month" value-format="yyyy-MM" placeholder="开始日期" :picker-options="common.startTime(fromData.EndTime)">
            </el-date-picker>
            -
            <el-date-picker v-model="fromData.EndTime" type="month" value-format="yyyy-MM" :disabled="fromData.IsToNow" placeholder="结束日期" :picker-options="common.endTime(fromData.StartTime)" :default-value="new Date()">
            </el-date-picker>
...
<script>
import { GetTrain, DelTrain, EditTrain } from "@/api/myResume";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      ruleValidate: {
        ...
        Institutions: [{ required: true, message: "请选择机构名称", trigger: "blur" }],
  ...
  methods: {
    ...
    getList() {
      GetTrain({ ResumeId: this.add_resume_id }).then((res) => {
        if (res.data.Success) {
