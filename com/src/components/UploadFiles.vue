<template>
  <el-row class="upload_files">
    <el-col :span="24" class v-if="false">
      <el-upload class="upload-demo" action="" :http-request="uploadFileMethod">
        <el-button class="plain_but">上传简历</el-button>
      </el-upload>
    </el-col>
    <el-dialog
    ...
    >
      ...
      >
        ...
        <el-form-item label="海外经历(留学)" prop="IsOverseaStudy">
          <!-- <el-radio v-model="OverseasStudy" label="StudyAbroad_02">是</el-radio>
            <el-radio v-model="OverseasStudy" label="StudyAbroad_01">否</el-radio> -->
          <el-select
            v-model="resumeData.IsOverseaStudy"
            placeholder="请选择"
            style="width: 100%"
          >
            <el-option
...
export default {
      ...
      SingleUpload(formData).then(res => {
        if (res.data.Success) {
          if (res.data.Data.ResultCode === 1) {
            ...
            this.$emit("onRepeat", res.data.Data);
          } else {
  ...
  mounted() {
    this.$Bus.$on("isUploadSubmit", data => {
